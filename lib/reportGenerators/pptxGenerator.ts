...
// ─── Palette ──────────────────────────────────────────────────────────────────
const P = {
  NAVY:    "0F1B2D",   // dominant dark
  INDIGO:  "1E3A5F",   // dark header bg
  STEEL:   "2563A8",   // accent blue
  ICE:     "CADCFC",   // light accent
  MINT:    "02C39A",   // positive / highlight
  AMBER:   "F59E0B",   // in-progress / warning
  ROSE:    "E11D48",   // risk / danger
  WHITE:   "FFFFFF",
  OFF:     "F7F9FC",   // slide background
...
function slideHeader(
  ...
  // Steel accent stripe
  slide.addShape(prs.ShapeType.rect, {
    x: 0, y: 0.95, w: 13.3, h: 0.06,
    fill: { color: P.STEEL },
    line: { color: P.STEEL },
  });
  slide.addText(title, {
    x: 0.45, y: 0, w: 9, h: 0.95,
    fontSize: 26, bold: true, color: P.WHITE, valign: "middle", margin: 0,
  });
  if (subtitle) {
    slide.addText(subtitle, {
      x: 0.45, y: 0, w: 12, h: 0.95,
      fontSize: 12, color: P.ICE, align: "right", valign: "middle", margin: 0,
    });
  }
}
...
function metricCard(
  ...
  slide.addText(value, {
    ...
    fontSize: 36, bold: true, color: accent, valign: "bottom",
  });
  slide.addText(label, {
    x: x + 0.18, y: y + 0.88, w: 2.7, h: 0.45,
    fontSize: 11, color: P.MUTED, valign: "top",
  });
}
...
export async function generatePptx(sprintData: any, ai: any, includeBlockerNotes: boolean = true): Promise<Buffer> {
  ...
  // ── Slide 1 — Cover ────────────────────────────────────────────────────────
  {
    ...
    // Large decorative circle (bottom-right bleed)
    s.addShape(prs.ShapeType.ellipse, {
      x: 9.5, y: 3.8, w: 5.5, h: 5.5,
      fill: { color: P.INDIGO },
      line: { color: P.INDIGO },
    });
    s.addShape(prs.ShapeType.ellipse, {
      x: 10.2, y: 4.5, w: 4.0, h: 4.0,
      fill: { color: P.STEEL, transparency: 70 },
      line: { color: P.STEEL, transparency: 70 },
    });

    // Mint accent bar left
    ...
    s.addText("SPRINT REPORT", {
      ...
      fontSize: 13, bold: true, color: P.ICE, charSpacing: 6,
    });
    s.addText(sprintData.sprintName, {
      x: 0.45, y: 2.05, w: 9, h: 1.5,
      fontSize: 52, bold: true, color: P.WHITE, wrap: true,
    });
    ...
    // Bottom stats row
    const bottomStats = [
      { v: String(sprintData.totalTasks),     l: "Total Tasks" },
      { v: String(sprintData.completedTasks), l: "Completed" },
      { v: String(sprintData.projects.length),l: "Projects" },
    ];
  ...
  // ── Slide 2 — Executive Summary ────────────────────────────────────────────
  {
    ...
    s.addText(ai.sprintSummary || "", {
      x: 0.7, y: 1.85, w: 7.3, h: 2.5,
      fontSize: 14, color: P.NAVY, wrap: true, valign: "top",
    });

    // Velocity note badge (right column)
    s.addShape(prs.ShapeType.rect, {
    ...
    s.addText("VELOCITY", {
      ...
      fontSize: 10, bold: true, color: P.ICE, charSpacing: 4,
    });
    s.addText(ai.velocityNote || "", {
      x: 8.7, y: 1.65, w: 4.0, h: 0.9,
      fontSize: 13, color: P.WHITE, wrap: true, italic: true,
    });

    // Completion rate donut-style stat
    const rate = Math.round((sprintData.completedTasks / sprintData.totalTasks) * 100);
    s.addShape(prs.ShapeType.ellipse, {
      x: 9.1, y: 2.95, w: 2.8, h: 2.8,
      fill: { color: P.INDIGO },
      line: { color: P.STEEL, pt: 3 },
      shadow: makeShadow(),
    });
    s.addText(`${rate}%`, {
      x: 9.1, y: 3.35, w: 2.8, h: 0.9,
      fontSize: 36, bold: true, color: P.MINT, align: "center",
    });
    s.addText("Completion\nRate", {
      x: 9.1, y: 4.25, w: 2.8, h: 0.6,
      fontSize: 11, color: P.ICE, align: "center",
    });
  }
  ...
  // ── Slide 3 — Sprint Metrics ────────────────────────────────────────────────
  {
    ...
    s.addChart(prs.ChartType.bar,
      ...
      }],
      {
        ...
        chartArea: { fill: { color: P.WHITE }, roundedCorners: false },
    ...
    // Mini summary panel (right)
    const summaryItems = [
      { label: "Sprint Duration", value: `${sprintData.startDate} → ${sprintData.endDate}` },
      { label: "Projects Active",  value: String(sprintData.projects.length) },
      { label: "Team Members",     value: String(sprintData.memberWorkload.length) },
    ];
  ...
  // ── Slide 5 — Team Performance ─────────────────────────────────────────────
  {
    ...
    // Workload bar chart (left)
    const members = sprintData.memberWorkload;
    s.addChart(prs.ChartType.bar,
      [
        { name: "Hours", labels: members.map((m: any) => m.name), values: members.map((m: any) => m.totalHours) },
        { name: "Tasks", labels: members.map((m: any) => m.name), values: members.map((m: any) => m.taskCount) },
      ],
      {
        x: 0.4, y: 1.2, w: 7.8, h: 5.9,
        barDir: "bar",
        barGrouping: "clustered",
        chartColors: [P.STEEL, P.ICE],
        chartArea: { fill: { color: P.WHITE }, roundedCorners: false },
        catAxisLabelColor: P.MUTED,
        valAxisLabelColor: P.MUTED,
        valGridLine: { color: P.BORDER, size: 0.5 },
  ...
  // ── Per-Project Slides ──────────────────────────────────────────────────────
  sprintData.projects.forEach((p: any) => {
    const s = prs.addSlide();
    s.background = { color: P.OFF };
    ...
    // Tasks table
    const headerRow = [
      { text: "Task", options: { bold: true, color: P.WHITE, fill: { color: P.NAVY }, fontSize: 11 } },
      { text: "Type", options: { bold: true, color: P.WHITE, fill: { color: P.NAVY }, fontSize: 11 } },
      { text: "Status", options: { bold: true, color: P.WHITE, fill: { color: P.NAVY }, fontSize: 11 } },
      { text: "Time", options: { bold: true, color: P.WHITE, fill: { color: P.NAVY }, fontSize: 11 } },
      { text: "Assignees", options: { bold: true, color: P.WHITE, fill: { color: P.NAVY }, fontSize: 11 } },
    ];
    const dataRows = tasks.map((t, ri) => {
      ...
      return [
        { text: t.title, options: { fill: { color: bg }, fontSize: 11, color: P.NAVY } },
        { text: t.type, options: { fill: { color: bg }, fontSize: 11, color: P.MUTED } },
        { text: t.status, options: { fill: { color: bg }, fontSize: 11, bold: true, color: statusC } },
        { text: `${t.timeValue}${t.timeUnit}`, options: { fill: { color: bg }, fontSize: 11, color: P.MUTED } },
        { text: t.assignees.join(", "), options: { fill: { color: bg }, fontSize: 11, color: P.MUTED } },
      ];
    });

    (s as any).addTable([headerRow, ...dataRows], {
      x: 0.4,
      y: 1.15,
      w: 12.5,
  ...
  // ── Final Slide — Recommendations + Next Sprint ─────────────────────────────
  {
    ...
    s.addText("RECOMMENDATIONS", {
      ...
      fontSize: 11, bold: true, color: P.ICE, charSpacing: 5,
    });
    ...
    (ai.recommendations || []).slice(0, 5).forEach((r: string, i: number) => {
      ...
      s.addText(r, {
        ...
        fontSize: 14, color: P.ICE, wrap: true, valign: "middle",
      });
    });
    ...
    s.addText("NEXT SPRINT", {
      ...
      fontSize: 11, bold: true, color: P.WHITE, charSpacing: 3, valign: "middle", margin: 0,
    });
    s.addText(ai.nextSprintFocus || "", {
      x: 9.4, y: 2.0, w: 3.3, h: 3.0,
      fontSize: 13, color: P.ICE, wrap: true, valign: "top",
    });
